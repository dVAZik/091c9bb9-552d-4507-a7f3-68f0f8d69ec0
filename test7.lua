-- Ultimate Farm Script - Final Fixed
-- Все функции: Kick + BP + Sell + Bonus + Speed + Weight + BuyW + Trade
-- Настраиваемые параметры, задержка вкл/выкл
...
-- ============ ПАРАМЕТРЫ ============
local KICK_POWER, KICK_READY_RADIUS, MIN_WAVE_DISTANCE = 10, 10, 300
local FINISH_DELAY_MIN, FINISH_DELAY_MAX = 0.3, 1.2
local FINISH_WAIT_MIN, FINISH_WAIT_MAX = 0.1, 0.8
local SWAY_AMOUNT, SWAY_CHANCE, STRAFE_CHANCE, JUMP_CHANCE = 8, 0.3, 0.15, 0.05
local WAVE_DANGER_DISTANCE, WAVE_CRITICAL_DISTANCE = 100, 30
local ENABLE_FINISH_DELAY = true

local WAVE_SPEEDS = {Common=11,Rare=18,Epic=32,Legendary=45,Mythic=55,Godly=68,Secret=80,Rainbow=90,Hacked=100,Demon=110,Celestial=125,Eternal=135,["Eternal+"]=155,Abyssal=165}
local TRADE_ITEMS = {"Ballberto", "Netini Goalini"}

-- ============ ПЕРЕМЕННЫЕ ============
local character, humanoid, rootPart, kickReadyPart, kickReadyPos, revKickEvent
...
local lastJumpTime, baseWalkSpeed, bpData, claimedCount = 0, 16, nil, 0
local selectedBrainrots, selectedMutations = {}, {["None"]=true}
local lastGUIUpdate, hasTeleported = 0, false

-- GUI
local screenGui, openButton, mainMenu, currentTab = nil, nil, nil, "Main"
local kickToggle, kickStatus, kickWaveLabel
local bpToggle, bpStatus, bpInfoLabel
local sellToggle, sellStatus, bonusToggle, bonusStatus
local speedToggle, speedStatus, weightToggle, weightStatus
local buyWeightToggle, buyWeightStatus, tradeToggle, tradeStatus
...
local function safeCPS(name)
	...
	local d = EntitiesData.Brainrots[name]; if not d or not d.CPS then return 0 end
	local raw = d.CPS
	local num = nil
	pcall(function() if type(raw)=="table" and raw.Value then num=tonumber(tostring(raw.Value):gsub("[^%d.]","")) end end)
	if not num then num=tonumber(tostring(raw):gsub(",",""):gsub("%s",""):gsub("[^%d.]","")) end
	if not num and type(raw)=="number" then num=raw end
	return num or 0
end
...
local function getCurrentMorph()
	...
	for _,c in ipairs(player.Character:GetChildren()) do
		if c:IsA("Tool") and c:HasTag("EntityTool") then
			return c.Name,c:GetAttribute("Mutation")or"None",c:GetAttribute("Level")or 1,safeCPS(c.Name)
		end
	end
	return "Нет","Нет",0,0
end
	...
	if player.Backpack then
		for _,item in ipairs(player.Backpack:GetChildren()) do
			if item:IsA("Tool") and item:HasTag("EntityTool") then
				...
				table.insert(inv[name],{Level=item:GetAttribute("Level")or 1,Mutation=item:GetAttribute("Mutation")or"None",GUID=item:GetAttribute("GUID"),CPS=safeCPS(name)})
			end
		end
	end
	return inv
...
local function findNetwork()
	...
	if s then local p = s:FindFirstChild("Packages"); if p then return p:FindFirstChild("Network") end end
	return nil
end
...
local function updateChar()
	...
	if character then
		...
		if humanoid and humanoid.Health>0 and rootPart then baseWalkSpeed=humanoid.WalkSpeed; return true end
	end
	return false
end
...
local function getDynamicSpeed()
	local dist,_,rarity = getClosestWave()
	local waveSpeed = WAVE_SPEEDS[rarity] or 25
	if dist >= WAVE_DANGER_DISTANCE then return baseWalkSpeed end
	if dist <= WAVE_CRITICAL_DISTANCE then return waveSpeed end
	local t = 1-(dist-WAVE_CRITICAL_DISTANCE)/(WAVE_DANGER_DISTANCE-WAVE_CRITICAL_DISTANCE)
	return baseWalkSpeed+(waveSpeed-baseWalkSpeed)*t
end

local function isCamOnPlayer()
	if not rootPart then return false end
	return (camera.CFrame.Position-rootPart.Position).Magnitude<50
end

local function waitForCam()
	local st=tick()
	while isKickActive and tick()-st<5 do
		if not updateChar() then return false end
		if isCamOnPlayer() then return true end
		task.wait(0.1)
	end
	return true
end
...
local function teleportToStart()
	...
	rootPart.CFrame = CFrame.new(kickReadyPos+Vector3.new(0,5,0))
	task.wait(0.3)
...
local function runToFinish()
	if not updateChar() then return false end
	if not humanoid or not rootPart or not kickReadyPos then return false end
	if humanoid.Health<=0 then return false end
	if isInZone() then return true end
	
	waitForCam()
	
	if ENABLE_FINISH_DELAY then
		task.wait(FINISH_DELAY_MIN+math.random()*(FINISH_DELAY_MAX-FINISH_DELAY_MIN))
	end
	...
	while isKickActive do
		...
		if math.random()<SWAY_CHANCE then swayDir=math.random(-1,1) end
		if math.random()<STRAFE_CHANCE then swayDir=math.random()<0.5 and -1 or 1 end
		
		local target = kickReadyPos
		if swayDir~=0 then
			local fwd = (kickReadyPos-rootPart.Position).Unit
			local right = Vector3.new(-fwd.Z,0,fwd.X)
			target = target+right*swayDir*SWAY_AMOUNT
		end
		humanoid:MoveTo(target)
		
		if math.random()<JUMP_CHANCE and tick()-lastJumpTime>0.5 then humanoid.Jump=true; lastJumpTime=tick() end
		if humanoid.MoveDirection.Magnitude<0.1 and tick()-lastJumpTime>0.5 then humanoid.Jump=true; lastJumpTime=tick() end
		task.wait(0.1)
	end
	return isInZone()
end
...
local function kickLoop()
	initKick()
	while isKickActive do
		if not updateChar() then task.wait(0.5); continue end
		local inGame, kd = player:GetAttribute("InGame"), player:GetAttribute("KickDebounced")
		
		if not isInZone() then
			if inGame==nil and kd==nil then if not hasTeleported then teleportToStart() end
			else hasTeleported=false; runToFinish() end
		end
		
		if isInZone() and inGame==nil and kd==nil then
			hasTeleported=false
			task.wait(FINISH_WAIT_MIN+math.random()*(FINISH_WAIT_MAX-FINISH_WAIT_MIN))
			if isInZone() and player:GetAttribute("InGame")==nil and player:GetAttribute("KickDebounced")==nil then
			...
			for _,item in ipairs(items) do
				if not(selectedBrainrots[name] and (selectedMutations[item.Mutation] or item.Mutation=="None")) and item.GUID then
					pcall(function()
						local net = findNetwork()
						if net then
							local ev = net:FindFirstChild("SELL_ENTITY")
							if ev then ev:FireServer(item.GUID) end
						end
					end)
					task.wait(0.05)
				end
				...
				if ku and ku.Enabled then
					for _,child in ipairs(ku:GetDescendants()) do
						if child:IsA("ImageButton") and (child.Name=="Bonus" or child.Name:find("Bonus")) and child.Visible and child.Active then
							...
							pcall(function() for _,b in ipairs(child.Activated:GetBindables()) do b:Fire() end end)
							task.wait(0.03)
						end
					end
				end
		...
		if WeightServiceClient and WeightsData then
			pcall(function()
				...
				for _,name in ipairs(WeightServiceClient.Owned or {}) do
					local d=WeightsData.Weights[name]
					if d and d.PPS and d.PPS>bestPPS then bestPPS=d.PPS; best=name end
				end
				if best and best~=WeightServiceClient.Equipped then
					local net=findNetwork()
					if net then local ev=net:FindFirstChild("WeightEquip"); if ev then ev:FireServer(best) end end
				end
			end)
		end
		task.wait(3)
...
local function autoTradeLoop()
	...
	local function findItem()
		...
		for _,item in ipairs(player.Backpack:GetChildren()) do
			if item:IsA("Tool") and item:HasTag("EntityTool") then
				for _,tn in ipairs(TRADE_ITEMS) do
					if item.Name==tn then return item:GetAttribute("GUID"),item,tn end
				end
			end
		end
	...
	pcall(function()
		Network.OnClientEvent("trade_n"):Connect(function(userId)
	...
	pcall(function()
		Network.OnClientEvent("trade_s"):Connect(function(status)
			...
			if status=="Trading" then
				if not itemAdded then task.wait(0.3); local guid=findItem(); if guid then pcall(function() Network.FireServer("trade_i","AddItem",guid) end); itemAdded=true end end
	...
	pcall(function()
		Network.OnClientEvent("trade_u"):Connect(function(data)
			...
			elseif data.Stage=="Trade" and not itemAdded then local guid=findItem(); if guid then pcall(function() Network.FireServer("trade_i","AddItem",guid) end); itemAdded=true end end
		end)
	end)
	...
	while isAutoTrade do
		...
		if guid and not tradeCompleted then
			if findTarget() and not tradeStarted then
...
local function applySettings()
	local function tn(s,d) if not s or s=="" then return d end; return tonumber(s) or d end
	if kickPowerInput then KICK_POWER=tn(kickPowerInput.Text,10) end
	if kickRadiusInput then KICK_READY_RADIUS=tn(kickRadiusInput.Text,10) end
	if minWaveDistInput then MIN_WAVE_DISTANCE=tn(minWaveDistInput.Text,300) end
	if delayMinInput then FINISH_DELAY_MIN=tn(delayMinInput.Text,0.3) end
	if delayMaxInput then FINISH_DELAY_MAX=tn(delayMaxInput.Text,1.2) end
	if waitMinInput then FINISH_WAIT_MIN=tn(waitMinInput.Text,0.1) end
	if waitMaxInput then FINISH_WAIT_MAX=tn(waitMaxInput.Text,0.8) end
	if swayAmountInput then SWAY_AMOUNT=tn(swayAmountInput.Text,8) end
	if swayChanceInput then SWAY_CHANCE=tn(swayChanceInput.Text,0.3) end
	if strafeChanceInput then STRAFE_CHANCE=tn(strafeChanceInput.Text,0.15) end
	if jumpChanceInput then JUMP_CHANCE=tn(jumpChanceInput.Text,0.05) end
	if waveDangerInput then WAVE_DANGER_DISTANCE=tn(waveDangerInput.Text,100) end
	if waveCriticalInput then WAVE_CRITICAL_DISTANCE=tn(waveCriticalInput.Text,30) end
	if delayToggleStatus then delayToggleStatus.Text=ENABLE_FINISH_DELAY and "Задержка: "..FINISH_DELAY_MIN.."-"..FINISH_DELAY_MAX.." сек" or "Задержка: ОТКЛЮЧЕНА" end
end
...
local function createGUI()
	...
	local tabNames={"⚽","⚙","🎒","🧠"}; local tabKeys={"Main","Settings","Inventory","Brainrot"}; local tabs={}
	...
	local function input(parent,y,defaultText)
		local tb=Instance.new("TextBox")
		tb.Size=UDim2.new(0.35,-16,0,26); tb.Position=UDim2.new(0.65,0,0,y)
		...
		tb.BorderSizePixel=0; tb.PlaceholderText=tostring(defaultText); tb.PlaceholderColor3=Color3.fromRGB(150,150,150)
		tb.Parent=parent; Instance.new("UICorner",tb).CornerRadius=UDim.new(0,4); return tb
	end
	...
	local sections={
		...
		{title="⬆ AUTO SPEED",color=Color3.fromRGB(150,255,150),text="SPEED",ref="isAutoSpeedUpgrade",loop=autoSpeedLoop},
	...
	for _,s in ipairs(sections) do
		refs[s.ref]=(function(n)return function(v)
			...
			elseif n=="isAutoBuyWeight"then isAutoBuyWeight=v elseif n=="isAutoTrade"then isAutoTrade=v end
		end end)(s.ref)
		states[s.ref]=(function(n)return function()
			...
			elseif n=="isAutoBuyWeight"then return isAutoBuyWeight elseif n=="isAutoTrade"then return isAutoTrade end
		end end)(s.ref)
	end
	...
	local ksSec=sec(setScroll,"⚽ УДАР",110,Color3.fromRGB(255,200,100))
	lbl(ksSec,"Сила удара:",26); kickPowerInput=input(ksSec,26,10)
	lbl(ksSec,"Радиус зоны:",56); kickRadiusInput=input(ksSec,56,10)
	lbl(ksSec,"Мин. дист. волны:",86); minWaveDistInput=input(ksSec,86,300)
	
	local mvSec=sec(setScroll,"🏃 ДВИЖЕНИЕ",200,Color3.fromRGB(150,255,150))
	lbl(mvSec,"Задержка MIN:",26); delayMinInput=input(mvSec,26,0.3)
	lbl(mvSec,"Задержка MAX:",56); delayMaxInput=input(mvSec,56,1.2)
	lbl(mvSec,"Ожидание MIN:",86); waitMinInput=input(mvSec,86,0.1)
	lbl(mvSec,"Ожидание MAX:",116); waitMaxInput=input(mvSec,116,0.8)
	lbl(mvSec,"Шатание (studs):",146); swayAmountInput=input(mvSec,146,8)
	lbl(mvSec,"Шанс шатания:",176); swayChanceInput=input(mvSec,176,0.3)
	
	local mvSec2=sec(setScroll,"🏃 ДВИЖЕНИЕ 2",100,Color3.fromRGB(150,255,150))
	lbl(mvSec2,"Шанс стрейфа:",26); strafeChanceInput=input(mvSec2,26,0.15)
	lbl(mvSec2,"Шанс прыжка:",56); jumpChanceInput=input(mvSec2,56,0.05)
	
	local delaySec=sec(setScroll,"⏱ ЗАДЕРЖКА",78,Color3.fromRGB(255,200,150))
	...
	delayToggleBtn.Text="⏹ ВКЛЮЧЕНА"; delayToggleBtn.BorderSizePixel=0; delayToggleBtn.Parent=delaySec
	Instance.new("UICorner",delayToggleBtn).CornerRadius=UDim.new(0,5)
	delayToggleStatus=lbl(delaySec,"Задержка: "..FINISH_DELAY_MIN.."-"..FINISH_DELAY_MAX.." сек",62)
	delayToggleBtn.MouseButton1Click:Connect(function()
		ENABLE_FINISH_DELAY=not ENABLE_FINISH_DELAY
		delayToggleBtn.Text=ENABLE_FINISH_DELAY and "⏹ ВКЛЮЧЕНА" or "▶ ВЫКЛЮЧЕНА"
		delayToggleBtn.BackgroundColor3=ENABLE_FINISH_DELAY and Color3.fromRGB(80,180,80) or Color3.fromRGB(180,50,50)
		delayToggleStatus.Text=ENABLE_FINISH_DELAY and "Задержка: "..FINISH_DELAY_MIN.."-"..FINISH_DELAY_MAX.." сек" or "Задержка: ОТКЛЮЧЕНА"
	end)
	
	local wvSec=sec(setScroll,"🌊 ВОЛНА",110,Color3.fromRGB(150,200,255))
	lbl(wvSec,"Дист. ускорения:",26); waveDangerInput=input(wvSec,26,100)
	lbl(wvSec,"Крит. дистанция:",56); waveCriticalInput=input(wvSec,56,30)
	
	local applySec=sec(setScroll,"💾 ПРИМЕНИТЬ",60,Color3.fromRGB(255,255,100))
	...
	applyBtn.TextSize=13; applyBtn.Font=Enum.Font.GothamBold; applyBtn.Text="💾 ПРИМЕНИТЬ"
	applyBtn.BorderSizePixel=0; applyBtn.Parent=applySec
	...
	task.spawn(function()
		while true do
			if mainMenu.Visible and tick()-lastGUIUpdate>0.3 then
				lastGUIUpdate=tick()
				if updateChar() then
					local dist,count,rarity=getClosestWave()
					kwv.Text=string.format("Волны: %d | %.0f studs | %s",count,dist,rarity)
					posinfo.Text="Позиция: "..(isInZone() and "В ЗОНЕ" or string.format("%.0f studs",kickReadyPos and (rootPart.Position-kickReadyPos).Magnitude or 0))
				end
...
-- ============ ЗАПУСК ============
createGUI()
player.CharacterAdded:Connect(function(c) character=c; task.wait(0.5); updateChar() end)
if player.Character then updateChar() end
getBPData()
print("Farm Menu loaded! All functions working.")
